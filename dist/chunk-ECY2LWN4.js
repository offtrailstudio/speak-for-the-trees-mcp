...
var usgsFeaturePropertiesSchema = z3.object({
  ...
  qualifier: z3.union([
    z3.string(),
    z3.array(z3.string()).transform((arr) => arr.join(", "))
  ]).nullish()
}).passthrough();
...
async function fetchWaterData(params = {}) {
  ...
  const readings = parsed.data.features.filter((feature) => feature.properties.value !== null).map((feature) => {
    const props = feature.properties;
...
var iNatTaxonSchema = z4.object({
  ...
  name: z4.string(),
  rank: z4.string(),
  iconic_taxon_name: z4.string().nullish(),
...
var attainsFeatureAttributesSchema = z6.object({
  ...
  reportingcycle: z6.union([z6.string(), z6.number()]).nullish().transform((v) => v != null ? String(v) : null),
